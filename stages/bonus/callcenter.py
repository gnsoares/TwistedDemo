...
class CallCenter():
    ...
    def call_to_operator(self, operator):
        ...
        if call is not None:
            ...
            timeout = 10
            d = task.deferLater(
                reactor, timeout, self.ignore_if_ringing, operator)

            def called(result):
                if result is not None:
                    print(result)
                    json_str = json.dumps({"response": result})
                    self.protocol.transport.write(json_str.encode('utf-8'))
    ...
    def answer(self, id):
        ...
        if operator is not None and operator.call is not None:
            call = operator.call
            operator.state = 'busy'
    ...
    def ignore_if_ringing(self, operator):
        ...
        if operator.state == 'ringing':
            ...
            message = f'Call {call.id} ignored by operator {operator.id}\n'
...
class CallCenterProtocol(Protocol):
    ...
    def __init__(self, call_center):
        ...
        self.call_center.protocol = self
    ...
    def dataReceived(self, data):
        ...
        command_pattern = re.compile(r'{"command": ".*", "id": ".*"}')
        mc = command_pattern.match(data.decode('utf-8'))

        if mc is not None:
            lines = mc.group(0).replace('>>> ', '').split('}')
            for line in lines[:-1]:
                self.processCommand(line + '}')

    def processCommand(self, json_str):

        command, arg = self.parseCommand(json_str)
    ...
    def parseCommand(self, json_str):
